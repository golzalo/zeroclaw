...
fn joined_policy_procedure_text(policy: &ObservedGroupConfig) -> String {
    [
...
fn whatsapp_policy_is_attachment_only_procedure(policy: &ObservedGroupConfig) -> bool {
    ...
    let text = joined_policy_procedure_text(policy);
    ...
    let storage_or_forwarding_action = [
        "upload",
        "subir",
        "sube",
        "suban",
    ...
    .iter()
    .any(|needle| text.contains(needle));
    let requires_semantic_image_processing = [
        "visual_analysis.v1",
    ...
    accepts_attachments && storage_or_forwarding_action && !requires_semantic_image_processing
}

fn document_marker_parts(line: &str) -> Option<(String, String)> {
...
fn attachment_filename(reference: &str, fallback: &str) -> String {
    Path::new(reference)
        .file_name()
...
fn attachment_mime(reference: &str, filename: &str) -> String {
    mime_guess::from_path(reference)
        .first_or_else(|| mime_guess::from_path(filename).first_or_octet_stream())
        .essence_str()
        .to_string()
}

fn current_turn_attachment_objects(content: &str) -> Vec<serde_json::Value> {
    let mut attachments = Vec::new();
    let mut seen_refs = HashSet::new();

    let (_, image_refs) = crate::multimodal::parse_image_markers(content);
    for image_ref in image_refs {
        let image_ref = image_ref.trim();
        if image_ref.is_empty() || !seen_refs.insert(image_ref.to_string()) {
            continue;
        }
        let filename = attachment_filename(image_ref, "image");
        attachments.push(serde_json::json!({
            "filename": filename,
            "mimeType": attachment_mime(image_ref, &filename),
            "path": image_ref,
            "localPath": image_ref,
        }));
    }
    ...
    for line in content.lines().map(str::trim) {
        let Some((name, reference)) = document_marker_parts(line) else {
            continue;
        };
        if !seen_refs.insert(reference.clone()) {
            continue;
        }
        let filename = attachment_filename(&reference, &name);
        attachments.push(serde_json::json!({
            "filename": filename,
            "mimeType": attachment_mime(&reference, &filename),
            "path": reference,
            "localPath": reference,
        }));
    }
...
fn policy_procedure_output_value(result: &tools::ToolResult) -> serde_json::Value {
    serde_json::from_str::<serde_json::Value>(&result.output).unwrap_or_else(|_| {
        serde_json::json!({
...
fn output_usize(value: &serde_json::Value, key: &str) -> Option<usize> {
    value
        .get(key)
...
fn output_names_from_array(value: &serde_json::Value, key: &str) -> Vec<String> {
    value
        .get(key)
            ...
            items
                ...
                .filter_map(|item| {
                    item.get("name")
                        ...
                        .or_else(|| item.get("source_file_name"))
...
fn render_attachment_only_policy_result(
    ...
    if !result.success {
        let detail = result
            ...
            .unwrap_or("el proceso no devolvió detalle del error");
        return format!("No pude subir los adjuntos. Error: {detail}");
    }

    let output = policy_procedure_output_value(result);
    let uploaded_count_value = output_usize(&output, "uploadedCount")
        .or_else(|| output_usize(&output, "uploaded_count"))
        .or_else(|| output_usize(&output, "rowsWrittenCount"));
    let uploaded_count = uploaded_count_value.unwrap_or(0);
    let failed_count = output_usize(&output, "failedCount")
        .or_else(|| output_usize(&output, "failed_count"))
        .unwrap_or_else(|| expected_count.saturating_sub(uploaded_count));
    let destination = output
        .get("destinationFolderName")
        .or_else(|| output.get("folderName"))
        .or_else(|| output.get("destination"))
        .and_then(|value| value.as_str())
        .map(str::trim)
        .filter(|value| !value.is_empty());
    let uploaded_names = output_names_from_array(&output, "uploads")
        .into_iter()
        .chain(output_names_from_array(&output, "uploadedFiles"))
        .collect::<Vec<_>>();
    let failed_names = output_names_from_array(&output, "failures")
        .into_iter()
        .chain(output_names_from_array(&output, "failedFiles"))
        .collect::<Vec<_>>();

    let destination_text = destination
        .map(|value| format!(" a {value}"))
        .unwrap_or_default();
    if uploaded_count_value.is_none() && uploaded_names.is_empty() {
        return format!(
            "El proceso terminó{destination_text}, pero no devolvió un conteo verificable. No voy a asumir que subió los {expected_count} adjunto(s)."
        );
    }
    if uploaded_count == expected_count && failed_count == 0 {
        if uploaded_names.is_empty() {
            format!("Listo: subí {uploaded_count} archivo(s){destination_text}.")
        } else {
            format!(
                "Listo: subí {uploaded_count} archivo(s){destination_text}: {}.",
                uploaded_names.join(", ")
            )
        }
    } else {
        let failed_text = if failed_names.is_empty() {
            String::new()
        } else {
            format!(" Fallaron: {}.", failed_names.join(", "))
        };
        format!(
            "Subí {uploaded_count} de {expected_count} archivo(s){destination_text}. Quedan {failed_count} sin subir.{failed_text}"
        )
    }
}

async fn maybe_handle_whatsapp_attachment_only_policy(
    ctx: &ChannelRuntimeContext,
    msg: &traits::ChannelMessage,
    target_channel: Option<&Arc<dyn Channel>>,
    policy: Option<&ObservedGroupConfig>,
) -> bool {
    let Some(policy) = policy else {
        return false;
    };
    if !whatsapp_policy_is_attachment_only_procedure(policy) {
        return false;
    }
    let attachments = current_turn_attachment_objects(&msg.content);
    ...
    let Some(channel) = target_channel else {
        tracing::warn!(
            ...
            "Cannot run WhatsApp attachment-only policy without target channel"
        );
        return false;
    };
    let Some(tool) = ctx
        .tools_registry
        .iter()
        .find(|tool| tool.name() == "whatsapp_run_policy_procedure")
    else {
        tracing::warn!(
            reply_target = msg.reply_target,
            "Cannot run WhatsApp attachment-only policy because whatsapp_run_policy_procedure is unavailable"
        );
        return false;
    };

    runtime_trace::record_event(
        "whatsapp_attachment_policy_direct_run",
        ...
        serde_json::json!({
            ...
            "job_slug": policy.procedure_job_slug.as_deref().unwrap_or(""),
        }),
    );

    let expected_count = attachments.len();
    let args = serde_json::json!({
        "chat_jid": msg.reply_target.as_str(),
        "input": {
    ...
    let reply = render_attachment_only_policy_result(expected_count, &tool_result);
    if let Err(err) = channel
        .send(
            &SendMessage::new(reply.clone(), &msg.reply_target).in_thread(msg.thread_ts.clone()),
        )
        .await
    {
        tracing::warn!(
            reply_target = msg.reply_target,
            "Failed to send WhatsApp attachment-only policy reply: {err}"
        );
    }
    true
}
...
async fn process_channel_message(
    ...
    if maybe_handle_whatsapp_attachment_only_policy(
        ctx.as_ref(),
        &msg,
        target_channel.as_ref(),
        whatsapp_conversation_policy.as_ref(),
    )
    .await
    {
        return;
...
mod tests {
    ...
    fn test_whatsapp_policy() -> ObservedGroupConfig {
        ObservedGroupConfig {
            ...
            enabled_at: "2026-05-19T00:00:00Z".to_string(),
            delivery_chat_jid: "120363025123456789@g.us".to_string(),
            ...
            procedure_summary: Some(
                "Upload every current-turn WhatsApp attachment to the configured Drive folder."
                    .to_string(),
            ),
            procedure_input_schema: Some(
                r#"{"type":"object","properties":{"attachments":{"type":"array"}}}"#
                    .to_string(),
            ),
    ...
    #[test]
    fn current_turn_attachment_objects_extracts_all_image_and_document_refs() {
        let attachments = current_turn_attachment_objects(
            "@s86 subi esto\n\
             [IMAGE:/workspace/attachments/whatsapp/a.jpg]\n\
             [Document: invoice.pdf] /workspace/attachments/whatsapp/invoice.pdf\n\
             [IMAGE:/workspace/attachments/whatsapp/a.jpg]",
        );
        ...
        assert_eq!(
            attachments[1]["path"],
            "/workspace/attachments/whatsapp/invoice.pdf"
        );
        assert_eq!(
            attachments[1]["localPath"],
            "/workspace/attachments/whatsapp/invoice.pdf"
        );
        assert_eq!(attachments[1]["filename"], "invoice.pdf");
        assert_eq!(attachments[1]["mimeType"], "application/pdf");
    }

    #[test]
    fn whatsapp_attachment_only_policy_detects_upload_contract() {
        let policy = test_whatsapp_policy();
        assert!(whatsapp_policy_is_attachment_only_procedure(&policy));

        let mut visual_policy = policy.clone();
        visual_policy.procedure_input_schema = Some(
            r#"{"type":"object","properties":{"visual_analysis":{"type":"object"}}}"#
                .to_string(),
        );
        visual_policy.procedure_input_contract =
            Some("Requires visual_analysis.v1 before writing the result.".to_string());
        assert!(!whatsapp_policy_is_attachment_only_procedure(
            &visual_policy
        ));
    }
