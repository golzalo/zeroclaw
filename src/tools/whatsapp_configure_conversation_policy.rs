...
impl WhatsAppConfigureConversationPolicyTool {
    ...
    fn normalize_policy_tools_arg(args: &serde_json::Value) -> anyhow::Result<Vec<String>> {
        let Some(value) = args.get("policy_tools") else {
            return Ok(Vec::new());
        };
        if value.is_null() {
            return Ok(Vec::new());
        ...
        let tools = value
            .as_array()
            .ok_or_else(|| anyhow::anyhow!("'policy_tools' must be an array of tool names"))?;
        ...
        for tool in tools {
            let name = tool
                ...
                .filter(|value| !value.is_empty())
                .ok_or_else(|| {
                    anyhow::anyhow!("'policy_tools' entries must be non-empty strings")
                })?;
    ...
    fn parse_procedure_metadata(
        ...
        let mut policy_tools = Self::normalize_policy_tools_arg(args)?;
        ...
        if procedure_job_slug.is_some() {
            ...
            if policy_tools.is_empty() {
                policy_tools.push("whatsapp_run_policy_procedure".to_string());
            } else if !policy_tools
                .iter()
                .any(|tool| tool == "whatsapp_run_policy_procedure")
            {
                anyhow::bail!(
                    "Procedure-backed WhatsApp policies must include \
                     `whatsapp_run_policy_procedure` in `policy_tools`."
                );
            }
        }
...
impl Tool for WhatsAppConfigureConversationPolicyTool {
        ...
        json!({
            ...
            "properties": {
                ...
                "policy_tools": {
                    "type": "array",
                    "items": { "type": "string" },
                    "description": "Explicit list of tool names the restricted worker may use in this conversation. When omitted for a procedure-backed policy, the runtime stores 'whatsapp_run_policy_procedure' automatically. If provided for a procedure-backed policy, it must include 'whatsapp_run_policy_procedure'."
                },
    ...
    async fn execute(&self, args: serde_json::Value) -> anyhow::Result<ToolResult> {
        ...
        if matches!(
            mode,
            ConversationMode::MentionReply | ConversationMode::ManagedGroup
        ) && procedure
            .as_ref()
    ...
    async fn configure_group_policy_uses_workspace_skill() {
        ...
        let result = tool
            .execute(json!({
                ...
                "goal": "Reply only when summoned and keep responses concise.",
                "reply_to_all": false
            }))
            .await
            .unwrap();
        ...
        assert_eq!(
            ...
            Some("Reply only when summoned and keep responses concise.")
        );
    }
    ...
    async fn configure_policy_with_reply_to_all_persists_field() {
        ...
        let result = tool
            .execute(json!({
                ...
                "delivery_chat_jid": "120363408016257691@g.us",
                "skill_name": "whatsapp_mention_reply",
                "goal": "Reply to all group messages with the mention-reply playbook.",
                "reply_to_all": true
            }))
            .await
            .unwrap();
